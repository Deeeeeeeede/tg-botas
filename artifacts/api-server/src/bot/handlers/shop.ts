...
export async function showPriceList(
  ctx: Context & { session: BotSession },
  opts: { forceQty?: boolean; backAction?: string } = {}
) {
  const cities = await getCities();
  const backAction = opts.backAction ?? "shop:home";

  if (cities.length === 0) {
    await editOrReplace(ctx, "No products available.", {
      ...inlineKeyboard([[BACK_BTN(backAction)]]),
    });
    return;
  }
  ...
  const showQty =
    opts.forceQty === true
      ? true
      : (await getSetting("show_pricelist_quantity")) !== "0";
  ...
  for (const city of cities) {
    ...
    for (const district of districts) {
      ...
      for (const row of rows) {
        const qtyPart = showQty ? `  × ${row.cnt}` : "";
        text += `    • ${row.typeEmoji} ${row.typeName} ${row.size} (${formatEur(row.price)})${qtyPart}\n`;
      }
    }
...
export async function showCustomerReviews(
  ...
  const [totalRow] = await db.select({ count: count() }).from(reviewsTable);
  ...
  const navRow: { text: string; callback_data: string }[] = [];
  if (page > 0)
    navRow.push({ text: "« Back", callback_data: `shop:view_reviews:${page - 1}` });
  if ((page + 1) * PAGE_SIZE < total)
    navRow.push({ text: "Show more reviews »", callback_data: `shop:view_reviews:${page + 1}` });
