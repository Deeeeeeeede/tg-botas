...
export async function showToolsMenu(ctx: Context & { session: BotSession }) {
  ...
  const [adminRow, jobEnabled, pricelistQty] = await Promise.all([
    ...
    getSetting("show_pricelist_quantity"),
  ]);
  ...
  const pricelistQtyBtn = pricelistQty === "0"
    ? { text: "📦 Show Stock in Pricelist: OFF", callback_data: "tools:toggle_pricelist_qty" }
    : { text: "📦 Show Stock in Pricelist: ON", callback_data: "tools:toggle_pricelist_qty" };
