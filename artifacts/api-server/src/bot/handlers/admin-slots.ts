...
export async function showCatalogType(
  ...
  if (slots.length === 0 && page === 0) {
    return showCatalog(ctx);
  }

  // Each slot gets two buttons: ✏️ edit price (full label) and 🗑 delete
  const slotRows: { text: string; callback_data: string }[][] = slots.map((s) => [
    {
      text: `✏️ ${s.cityName}·${s.distName}·${s.size} — ${formatEur(s.price)}`,
      callback_data: `eprod:cat_edit_price:${s.id}:${typeId}:${page}`,
    },
    {
      text: "🗑",
      callback_data: `eprod:cat_del:${s.id}:${typeId}:${page}`,
    },
  ]);

  const navRow: { text: string; callback_data: string }[] = [];
  if (page > 0) navRow.push({ text: "« Prev", callback_data: `eprod:cat_type:${typeId}:${page - 1}` });
  if ((page + 1) * PAGE_SIZE < total) navRow.push({ text: "Show more »", callback_data: `eprod:cat_type:${typeId}:${page + 1}` });

  const kb = inlineKeyboard([
    ...slotRows,
    ...(navRow.length ? [navRow] : []),
    [{ text: "🗑 Remove ALL catalog slots", callback_data: `eprod:cat_delall:${typeId}` }],
    [BACK_BTN("eprod:catalog")],
  ]);

  await ctx.editMessageText(
    `📋 <b>Catalog Slots</b> (${total} total)\n\n` +
    `✏️ Tap a slot to edit its price  •  🗑 Tap 🗑 to delete.\n` +
    `<b>Uploaded products are NOT affected.</b>`,
    { parse_mode: "HTML", ...kb },
  );
}
