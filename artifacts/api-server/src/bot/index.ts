...
export function createBot(token?: string): Telegraf {
  ...
  bot.on(message("text"), async (ctx: any) => {
    ...
    if (step === "admin:slot_edit_price") {
      const slotId = data["slotId"] as number;
      const typeId = data["typeId"] as number;
      ...
      await db
        .update(productSlotsTable)
        .set({ price: newPrice.toFixed(2) })
        .where(eq(productSlotsTable.id, slotId));
      ctx.session.step = undefined;
      ctx.session.data = undefined;
      await ctx.reply(`✅ Slot price updated to ${formatEur(newPrice.toFixed(2))}.`);
      await showCatalogType(ctx, typeId, page);
      return;
    }
  ...
  bot.on("callback_query", async (ctx: any) => {
    ...
    try {
      ...
      if (action === "eprod") {
        ...
        if (sub === "cat_type")
          return showCatalogType(ctx, parseInt(parts[1]!), parseInt(parts[2] ?? "0"));
        if (sub === "cat_del")
        ...
        if (sub === "cat_edit_price") {
          const slotId = parseInt(parts[1]!);
          const typeId = parseInt(parts[2]!);
          const page = parseInt(parts[3] ?? "0");
          ctx.session.step = "admin:slot_edit_price";
          ctx.session.data = { slotId, typeId, page };
          return ctx.editMessageText(
            "💰 Enter the new price (EUR) for this slot (e.g. 25.00):",
            inlineKeyboard([[BACK_BTN(`eprod:cat_type:${typeId}:${page}`)]]),
          );
        }
        if (sub === "new_type") {
      ...
      if (action === "disc") {
        ...
        if (sub === "prod_disc_scope") {
          ...
          if (scope === "all" || scope === "type") {
            ...
            const cities = await getCities();
            const kb = inlineKeyboard([
              [{ text: "🌍 All cities", callback_data: "disc:prod_disc_city:all" }],
              ...cities.map((c) => [
                { text: `🏙 ${c.name}`, callback_data: `disc:prod_disc_city:city:${c.id}` },
              ]),
              [BACK_BTN("disc:add_product_disc")],
            ]);
            return ctx.editMessageText(
              "🏙 Select city for this discount, or choose all cities:",
              kb,
            );
          }
          return;
        }
        if (sub === "prod_disc_city") {
          const cityScope = parts[1];
          if (cityScope === "city") {
            ctx.session.data = {
              ...(ctx.session.data ?? {}),
              cityId: parseInt(parts[2]!),
      ...
      if (action === "tools") {
        ...
        if (sub === "toggle_pricelist_qty") {
          const current = await getSetting("show_pricelist_quantity");
          const next = current === "0" ? "1" : "0";
          await setSetting("show_pricelist_quantity", next);
          await ctx.answerCbQuery(next === "0" ? "Stock hidden from pricelist." : "Stock shown in pricelist.");
          return showToolsMenu(ctx);
        }
        if (sub === "job_toggle") {
      ...
      if (action === "shop") {
        ...
        if (sub === "view_reviews") {
          const page = parseInt(parts[1] ?? "0");
          return showCustomerReviews(ctx, page);
        }
        return;
      }
