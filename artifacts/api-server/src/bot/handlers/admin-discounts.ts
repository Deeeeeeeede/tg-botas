...
export async function showProductDiscounts(ctx: Context & { session: BotSession }) {
  const discounts = await db
    .select({
      id: productDiscountsTable.id,
      cityId: productDiscountsTable.cityId,
      districtId: productDiscountsTable.districtId,
      typeId: productDiscountsTable.typeId,
      size: productDiscountsTable.size,
      percentOff: productDiscountsTable.percentOff,
      cityName: citiesTable.name,
      typeName: productTypesTable.name,
    })
    .from(productDiscountsTable)
    .leftJoin(citiesTable, eq(productDiscountsTable.cityId, citiesTable.id))
    .leftJoin(productTypesTable, eq(productDiscountsTable.typeId, productTypesTable.id))
  ...
  if (discounts.length === 0) {
    ...
    for (const d of discounts) {
      const scope = [
        d.typeName ?? null,
        d.cityName ? `📍 ${d.cityName}` : null,
        d.size ? `${d.size}` : null,
      ]
        .filter(Boolean)
        .join(" / ");
      text += `• ${scope || "All products"} — ${d.percentOff}% off\n`;
    }
  }
  ...
  const kb = inlineKeyboard([
    ...
    ...discounts.map((d) => [
      {
        text: `🗑 ${[d.typeName, d.cityName, d.size].filter(Boolean).join(" / ") || "All"} (${d.percentOff}%)`,
