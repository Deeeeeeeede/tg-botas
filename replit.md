...
- Payment→invoice binding: each open invoice (purchase and top-up) gets a *guaranteed-unique* SOL amount via `makeUniqueSolAmount`, and exact on-chain matching uses a tight tolerance (`MATCH_TOL` = 2e-6, strictly < half the 1e-5 uniqueness step) so each invoice's acceptance window is disjoint. A payment therefore matches at most one open invoice — one buyer's payment can never satisfy another's order. `makeUniqueSolAmount` is synchronous and reserves its amount before returning to close the create-time race. The paying wallet is captured (`purchasesTable.senderWallet`, biggest balance-drop account) and shown in Today's Sales.
- Non-exact purchase payments (customers who don't read and just send a rounded/approximate amount): the invoice text still shouts "SEND THE EXACT AMOUNT", but the bot no longer strands a non-exact payment. PURCHASES use a fuzzy acceptance tier (`acceptsPayment`, `ACCEPT_TOL` = 0.02 SOL) gated through `scanForPayment(..., { allowFuzzy: true })` — accepted only when the payment is within `ACCEPT_TOL` of the invoice **and** of exactly ONE live invoice (strictly amount-bound, so an unrelated inbound deposit or another buyer's order is never hijacked). TOP-UPS stay exact (no `allowFuzzy`). Under/over is judged in **SOL** (sent vs asked), never EUR, so a SOL price move can't misclassify an exact payment. Overpay → product delivered, the extra credited to balance (`overpaySol * price`). Underpay → product NOT delivered; `handleUnderpaymentToBalance` credits exactly what they sent to balance (atomic with the UNIQUE `tx_signature` claim) and tells them to send the exact amount next time; if the SOL price feed is down (price ≤ 0) it skips without claiming so the next tick retries (never consume a signature without crediting). Payments more than `ACCEPT_TOL` off match nothing live and fall to reconciliation / admin recovery. `reconcilePayments()` stays tight (`MATCH_TOL`); late non-exact payments are recovered via the admin Unmatched Payments tool.
